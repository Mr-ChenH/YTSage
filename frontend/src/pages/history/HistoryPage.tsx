...
function historyMetadataBoolean(entry: HistoryEntry, key: string): boolean {
  return entry.metadata[key] === true;
}

function historyPlaylistCount(entry: HistoryEntry): number {
  const value = entry.metadata.playlist_count;
  return typeof value === 'number' && Number.isFinite(value) ? value : 0;
}

function historySummary(entry: HistoryEntry, t: T): string {
  const count = historyPlaylistCount(entry);
  return count > 0 ? `${count} ${t('playlistItems')} · ${formatBytes(entry.file_size)}` : formatBytes(entry.file_size);
}
